...
> 状态：实验执行中；标准 openGauss 6.0.0 JSONB 矩阵和裁剪后的 ClickHouse 机制矩阵已完成
> 日期：2026-09-04
...
预算边界、混合密度和等单行宽度组已经完成。紧接的路径工作是完成真实 Trace 窗口审计，再决定 `50/500 × 1%/5%/20%/50%/95%` 中的代表性主梯度；2% 和 10% 只细化已发现的转折。面向目标实时分析负载的后续布局比较包括“强类型列 + String residual”“强类型列 + Map”和“强类型列 + 有路径预算的 native JSON”，并加入持续写入、后台 merge 与并发查询。长字段同时比较同表独立列、独立 payload 表、Full/Core 物化和 asset reference。九组均匀密度产物保留为机制资产；`5000×20%` 和 `5000×95%` 缺少场景依据，不进入当前结论。
