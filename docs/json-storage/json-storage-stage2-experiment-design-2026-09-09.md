...
实验覆盖真实 Trace 分布审计、持续分批写入、写入期间并发查询、后台维护、静态查询和空间统计。重点回答三类问题：JSON 列之外的独立列查询是否受动态属性结构影响、路径查询如何受索引或子列组织影响、完整 Trace 回查需要承担哪些读取和恢复成本。
...
4. 按独立列、路径过滤和完整 Trace 回查组织的横向结果；
5. 正确性、原文恢复和异常；
