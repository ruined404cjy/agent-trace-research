...
阶段子进程为每个请求流 fork 一个独立进程，运行该流的到达调度与 worker，各流不共享解释器锁与调度线程。流进程在预热与测量两段内持续存在，两段各使用阶段子进程选定的同一 monotonic 起点，结果经 Pipe 回传。流进程异常退出、超出结果上界或存在未收束请求时，阶段以带流名的未收束执行失败并保留 namespace。phase manifest 的 `stream_execution` 字段记录该执行模型，正式结果要求其值为 `process_per_stream`。
